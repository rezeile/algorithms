/*
 * Implementation of the merge sort algorithm for sortin an array of templetized
 * items. The idea for an abstract in-place merge, one in which we appear to 
 * sort and merge the arrays in place (rather than using an auxilary array) was 
 * taken from Robert Sedgewick and Kevin Wayne's Introduction to Algorithms, 
 * Third Edition (Chapter 2.3 Merge Sort). 
 *
 * 
 * THE FOLLOWING IMPLEMENTATION IS NOT COMPLETE YET!
 *
 * Author: Eliezer Abate
 * Last Edited: Dec 25 2015 18:48 PST
 */
  ...
  */
  ...
  void MergeSort(std::vector<T> &elems, Comparator comp) {
    ...
    detail::sort(elems,0, elems.size() / 2, elems.size() - 1, comp,aux);
  }
